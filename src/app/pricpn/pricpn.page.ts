...
@Component({
  selector: 'app-pricpn',
  templateUrl: './pricpn.page.html',
  styleUrls: ['./pricpn.page.scss'],
})
export class PricpnPage implements OnInit {
  loading:any
  sub:Subscription
  page=1;
  limit=5
  ...
  getPriCpn(event?:any){
    //get news again
  this.sub = this.ds.getPriCpn(this.page,this.limit).subscribe((res:any)=>{
    ...
    this.Results=this.Results.concat(res.data.pricpn);
