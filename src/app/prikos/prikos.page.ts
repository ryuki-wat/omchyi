...
@Component({
  selector: 'app-prikos',
  templateUrl: './prikos.page.html',
  styleUrls: ['./prikos.page.scss'],
})
export class PrikosPage implements OnInit {
  ...
  getPriKos(event?:any){
    //get news again
  this.sub = this.ds.getPriKos(this.page,this.limit).subscribe((res:any)=>{
     
     this.pageTotal=res.data.totalPages
     this.totalItem=res.data.totalItems
    

    this.Results=this.Results.concat(res.data.prikos);
