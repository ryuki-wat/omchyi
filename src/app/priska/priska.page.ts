...
@Component({
  selector: 'app-priska',
  templateUrl: './priska.page.html',
  styleUrls: ['./priska.page.scss'],
})
export class PriskaPage implements OnInit {
  loading:any
  sub:Subscription
  page=1;
  limit=5
  ...
  getPriSka(event?:any){
    //get news again
  this.sub = this.ds.getPriSka(this.page,this.limit).subscribe((res:any)=>{
    ...
    this.Results=this.Results.concat(res.data.priska);
