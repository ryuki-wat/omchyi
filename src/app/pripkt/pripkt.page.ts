...
@Component({
  selector: 'app-pripkt',
  templateUrl: './pripkt.page.html',
  styleUrls: ['./pripkt.page.scss'],
})
export class PripktPage implements OnInit {
  loading:any
  sub:Subscription
  page=1;
  limit=5
  ...
  getPriPkt(event?:any){
    //get news again
  this.sub = this.ds.getPriPkt(this.page,this.limit).subscribe((res:any)=>{
    ...
    this.Results=this.Results.concat(res.data.pripkt);
