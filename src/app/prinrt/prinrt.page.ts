...
@Component({
  selector: 'app-prinrt',
  templateUrl: './prinrt.page.html',
  styleUrls: ['./prinrt.page.scss'],
})
export class PrinrtPage implements OnInit {
  loading:any
  sub:Subscription
  page=1;
  limit=5
  ...
  getPriNrt(event?:any){
    //get news again
  this.sub = this.ds.getPriNrt(this.page,this.limit).subscribe((res:any)=>{
    ...
    this.Results=this.Results.concat(res.data.prinrt);
