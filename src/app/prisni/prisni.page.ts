...
@Component({
  selector: 'app-prisni',
  templateUrl: './prisni.page.html',
  styleUrls: ['./prisni.page.scss'],
})
export class PrisniPage implements OnInit {
  loading:any
  sub:Subscription
  page=1;
  limit=5
  ...
  getPriSni(event?:any){
    //get news again
  this.sub = this.ds.getPriSni(this.page,this.limit).subscribe((res:any)=>{
    ...
    this.Results=this.Results.concat(res.data.prisni);
